...
const generateWAMessageContent = async (message, options) => {
    ...
    if ('mentionAll' in message && message.mentionAll) {
        ...
        m[messageType].contextInfo.groupMentions = [
            {
                groupJid: options.cachedGroupMetadata ? options.cachedGroupMetadata(options.jid || '').id : options.jid,
                groupSubject: options.cachedGroupMetadata ? options.cachedGroupMetadata(options.jid || '').subject : 'Group'
            }
        ];
    }
    if ('edit' in message) {
